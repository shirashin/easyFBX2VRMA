...
- 📦 **一括変換機能** - 複数のFBXファイルを同時に変換してZIPファイルで出力
- 📊 **リアルタイム進捗表示** - 変換の進行状況を視覚的に確認
- 🎬 **アニメーションプレビュー** - 変換後のVRMAアニメーションをその場で確認可能
- 🔄 **アニメーション切り替え** - 複数のアニメーション間をワンクリックで切り替え
- 👤 **キャラクター変更** - カスタムVRMキャラクターでアニメーションを確認
- 🎨 **背景色カスタマイズ** - プレビュー背景色をお好みに変更
...
1. FBXファイルをアプリのウィンドウにドラッグ&ドロップ
   - または「クリックしてファイルを選択」から単一のFBXファイルを選択
2. 自動で変換が開始され、進捗バーが表示されます
3. 変換完了後、保存先を選択してVRMAファイルを保存

#### 一括変換
1. 複数のFBXファイルを同時にドラッグ&ドロップ（最大30ファイル）
2. 一括変換進捗画面で各ファイルの変換状況を確認
3. 変換ログで詳細な処理状況をリアルタイムで確認
4. 完了後、ZIPファイルとしてまとめて保存

#### アニメーションプレビュー機能
- **プレビュー画面**: 変換後のVRMAアニメーションをリアルタイムで確認
- **アニメーション制御**: Play/Stopボタンでアニメーションを再生・停止
- **ファイル切り替え**: 複数のアニメーションがある場合、ボタンで簡単に切り替え
- **キャラクター変更**: お好みのVRMファイルを読み込んでアニメーションを確認
- **背景色変更**: プリセットまたはカスタムカラーで背景色を変更
- **3D操作**: マウスでカメラアングルを自由に変更
...
- 一括変換で一部ファイルが失敗する場合は、変換ログで詳細なエラー内容を確認
...
- 変換が正常に完了しているか確認
- アプリを再起動してみてください
- VRMキャラクターを変更した場合は、アニメーションを再読み込みしてください

### 一括変換の制限
- 最大30ファイルまで同時変換可能です
- メモリ使用量が多い場合は、分割して変換することをお勧めします
